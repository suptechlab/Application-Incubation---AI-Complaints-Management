...
export const handleGetInquiryType = async (params) => {
  return await adminApi.get(`/${API_VERSION}/inquiry-types`, {
    params
  });
}
...
export const createNewInquiryType = async (data) => {
  return await adminApi.post(`/${API_VERSION}/inquiry-types`,data);
}

// EDIT EXISTING INQUIRY TYPE
export const editInquiryType = async (id,data) => {
  return await adminApi.put(`/${API_VERSION}/inquiry-types/${id}`,data);
}

// GET INQUIRY TYPE BY ID
export const getInquiryTypeById = async (id) => {
  return await adminApi.get(`/${API_VERSION}/inquiry-types/${id}`);
}

// UPDATE INQUIRY TYPE STATUS
export const changeInquiryTypeStatus = async (id,status) => {
  return await adminApi.patch(`/${API_VERSION}/inquiry-types/${id}/status?status=${status}`);
}

// EXPORT INQUIRY TYPE LIST
export const downloadInquiryTypes = async (params) => {
  return await adminApi.get(`/${API_VERSION}/inquiry-types/download`, { params, responseType: 'arraybuffer' });
}
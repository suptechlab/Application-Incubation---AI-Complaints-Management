...
public class DocumentServiceImpl implements DocumentService {
    ...
    private static final Set<String> SUPPORTED_FILE_TYPES = Set.of("jpg", "jpeg", "png", "pdf", "docx", "txt");
    private static final String CLAIM_TRANSACTION_TYPE = "CLAIM";
    private static final int MAX_FILE_NAME_LENGTH = 254;

    public ResponseEntity<String> uploadDocument(byte[] fileBytes, String idTicket, String uniqueFileName) {
        String fileExtension = getFileExtension(uniqueFileName);
        if (!SUPPORTED_FILE_TYPES.contains(fileExtension)) {
            String errorMessage = messageSource.getMessage("error.unsupported.file.type", new Object[]{fileExtension}, LocaleContextHolder.getLocale());
            LOG.error("Unsupported file type: {}", fileExtension);
            throw new InvalidFileTypeException(errorMessage);
        }
        Map<String, Object> payload = new HashMap<>();
        payload.put("nombreDocumento", uniqueFileName);
        payload.put("tipoTransaccion", CLAIM_TRANSACTION_TYPE);
